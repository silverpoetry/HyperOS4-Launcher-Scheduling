...
Invoke-Adb @('-s', $Serial, 'shell', 'su', '-c',
    "chmod 0755 $stage/bin/launcher-logwatch $stage/bin/source-guard $stage/deploy-runtime.sh")
...
for ($attempt = 0; $attempt -lt 20; $attempt++) {
    $daemonPid = & $Adb -s $Serial shell su -c "test -S /dev/.hyperos4-launcher-scheduling/source-guard.sock && grep -q '^online=1$' /dev/.hyperos4-launcher-scheduling/coordinator.status && cat $moduleDirectory/daemon.pid" 2>$null
